...
intended_dir = gears.filesystem.get_configuration_dir() .."theme.lua"
if not beautiful.init(intended_dir) then
    naughty.notify({
        preset = naughty.config.presets.critical,
        title= "Theme Error",
        text="Unable to load correct theme, rolling back to default\n".. "Intended path:"..intended_dir
    })
...
local function set_wallpaper(s)
    -- Wallpaper, set using the beautiful wallpaper value
...
globalkeys = gears.table.join(
    ...
    -- Standard program
    -- Always spawn terminals as slaves
    awful.key({ modkey,           }, "Return", function () awful.spawn(terminal, { callback = awful.client.setslave}) end,
...
client.connect_signal("request::titlebars", function(c)
    -- buttons for the titlebar, click to move, right click to resize
...
client.connect_signal("mouse::enter", function(c)
    -- Tell client that it is activated, and to raise to top of stack
...
awful.screen.connect_for_each_screen(function (s) 
    s:connect_signal("tag::history::update",function()
        ...
        for k,c in pairs(clients) do
            if s.selected_tag.layout.name == "floating" then
                if not c.requests_no_titlebar then
                    awful.titlebar.show(c)
                end
            else 
...
--Listen for changes for all tags for changes in the layout of the tag
